...
namespace YAAL
{
    public static partial class IOManager
    {
        ...
        {
            string path = Path.Combine(GetSaveLocation(ManagedApworlds), gameName, versions.GetFileName());
            Cache_Versions cache = LoadCache<Cache_Versions>(path);
            ...
            if (cache.versions.ContainsKey(version))
            {
                ...
                SaveCache<Cache_Versions>(path, cache);
            } else
            {
                ...
                SaveCache<Cache_Versions>(path, newCache);
            }
        }
        ...
        public static void CreateNewVersionCache(string gameName, string version) {
            ...
            SaveCache<Cache_Versions>(fullPath, cache);
        }
        ...
        {
            string path = Path.Combine(GetSaveLocation(ManagedApworlds), gameName, FileSettings.versions.GetFileName());
            Cache_Versions cache = LoadCache<Cache_Versions>(path);
        ...
        {
            string path = Path.Combine(GetSaveLocation(ManagedApworlds), game, versions.GetFileName());
            Cache_Versions cache = LoadCache<Cache_Versions>(path);
        ...
        public static void RemoveVersion(string game, string version)
        {
            ...
            HardDeleteFile(path);
            string cacheJson = Path.Combine(GetSaveLocation(ManagedApworlds), game, versions.GetFileName());
            if (File.Exists(cacheJson))
            {
                Cache_Versions cache = LoadCache<Cache_Versions>(cacheJson);
                if (cache.downloaded.ContainsKey(version))
                {
                ...
                SaveCache<Cache_Versions>(cacheJson, cache);
            }
        }
        ...
        {
            string path = Path.Combine(GetSaveLocation(ManagedApworlds), gameName, versions.GetFileName());
            string versionDirectory = Path.Combine(GetSaveLocation(ManagedApworlds), gameName, version);
        ...
        {
            string workingName = GetFileName(fileToCopy);


            string output = Path.Combine(
                GetSaveLocation(ManagedApworlds),
                gameName,
                version,
                workingName
                );
